...
class PHogFeatures():
    ...
    def get_features(self, image_path,g, bins=8, angle=360., pyramid_levels=3):
        """
        ...
        feature_vec = self.phog(image_path,g, bins, angle, pyramid_levels)
        feature_vec = feature_vec.T[0]  # Transpose vector, take the first array
        collection.insert({"ID":g,"descriptor":feature_vec.tolist()}) 

    def phog(self, image_path,g, bin, angle, pyramid_levels):
        """
        ...
        bh_roi = bh
        bv_roi = bv
        p = self.phog_descriptor(g,bh_roi, bv_roi, pyramid_levels, bin)
    ...
    def phog_descriptor(self, g,bh, bv, pyramid_levels, bin):
        """
...
while(i<500):
	j=str(i)
	#k=g+1
	#img=cv2.imread("/home/ubuntu/coil-20/obj5__"+j+".png",0)
	
	var="/home/ubuntu/Downloads/UCIDPNG/"+j+".png"
	#print var
	hit=PHogFeatures()
	hit.get_features(var,g)
